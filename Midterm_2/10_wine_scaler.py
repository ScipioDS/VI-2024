...
if __name__ == '__main__':
    ...
    col_index = features_importances.index(min(features_importances))

    dataset = [row[:col_index] + row[col_index+1:] for row in dataset]

    train_set_col = dataset[int(percent * len(dataset)):]
    test_set_col = dataset[:int(percent * len(dataset))]

    train_x_col = [row[:-1] for row in train_set_col]
    train_y_col = [row[-1] for row in train_set_col]

    test_x_col = [row[:-1] for row in test_set_col]
    test_y_col = [row[-1] for row in test_set_col]

    classifier_1 = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    classifier_2 = MLPClassifier(hidden_layer_sizes=15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)

    standard_scaler = StandardScaler()
    standard_scaler.fit(train_x_col)

    minmax_scaler = MinMaxScaler()
    minmax_scaler.fit(train_x_col)

    classifier_1.fit(standard_scaler.transform(train_x_col), train_y_col)
    classifier_2.fit(minmax_scaler.transform(train_x_col), train_y_col)

    standard_score = classifier_1.score(standard_scaler.transform(test_x_col), test_y_col)
    print(f'Tocnost so StandardScaler: {standard_score}')

    minmax_score = classifier_2.score(minmax_scaler.transform(test_x_col), test_y_col)
    print(f'Tocnost so MinMaxScaler: {minmax_score}')

...
class GhostOnSkates(Problem):
    ...
    def check_valid(state, walls, n):
        ghost_post = list(state)
        if ghost_post in walls:
            return False
        if ghost_post[0] >= n or ghost_post[0] < 0:
            return False
        if ghost_post[1] >= n or ghost_post[1] < 0:
            return False
        return True

    def successor(self, state):
        successors = dict()
        directions = ("Gore 1", "Gore 2", "Gore 3", "Desno 1", "Desno 2", "Desno 3")
        for direction in directions:
            new_state = move_ghost(state, direction)
    ...
    def h(self, node):
        ...
        goal = list(self.goal)
        return abs(ghost[0] - goal[0]) + abs(ghost[1] - goal[1])


def move_ghost(state, direction):
    # Gore/desno

    ghost = list(state)
    if direction == "Gore 1":
        ghost[1] += 1
    elif direction == "Gore 2":
        ghost[1] += 2
    elif direction == "Gore 3":
        ghost[1] += 3
    elif direction == "Desno 1":
        ghost[0] += 1
    elif direction == "Desno 2":
        ghost[0] += 2
    elif direction == "Desno 3":
        ghost[0] += 3
    return tuple(ghost)
...
if __name__ == '__main__':
    ...
    if astar_search(problem) is not None:

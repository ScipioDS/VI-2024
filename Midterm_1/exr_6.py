...
class Squares(Problem):
    ...
    def successor(self, state):
        successors = dict()
        actions = ("levo", "desno", "gore", "dolu")
        for i in range(5):
            for action in actions:
                new_state = move_square(state, i, action)
                if self.check_valid(new_state):
                    successors[f'Pomesti kvadratche {i+1} {action}'] = new_state
        return successors
...
def move_square(state, index, direction):
    squares = list(state)

...
if __name__ == '__main__':
    ...
    train_set = zero_set[:int(len(zero_set) * percent)] + one_set[:int(len(one_set) * percent)]
    test_set = zero_set[int(len(zero_set) * percent):] + one_set[int(len(one_set) * percent):]
    ...
    score_NB = classifier_NB.score(test_x, test_y)

    classifier_TR = DecisionTreeClassifier(criterion='entropy', random_state=0)
    classifier_TR.fit(train_x, train_y)
    score_TR = classifier_TR.score(test_x, test_y)

    classifier_FR = RandomForestClassifier(n_estimators=4, criterion='entropy', random_state=0)
    classifier_FR.fit(train_x, train_y)
    score_FR = classifier_FR.score(test_x, test_y)

    classifier_NN = MLPClassifier(hidden_layer_sizes=(10,), activation='relu', learning_rate_init=0.001, random_state=0)
    classifier_NN.fit(train_x, train_y)
    score_NN = classifier_NN.score(test_x, test_y)

    lst = [(1, score_NB), (2, score_TR), (3, score_FR), (4, score_NN)]
    lst_sort = sorted(lst, key=lambda x: x[1], reverse=True)

    best_scorer = lst_sort[0][0]
    ...
    TP = 0  # correct - positive
    FP = 0  # incorrect - positive
    TN = 0  # correct - negative
    FN = 0  # incorrect - negative

    for i in range(len(test_set)):
        actual = test_y[i]
        predicted_NB = classifier_NB.predict([test_x[i]])[0]
        predicted_TR = classifier_TR.predict([test_x[i]])[0]
        predicted_FR = classifier_FR.predict([test_x[i]])[0]
        predicted_NN = classifier_NN.predict([test_x[i]])[0]

        list_scores = [predicted_NB, predicted_TR, predicted_FR, predicted_NN]

        zero = 0
        one = 0
        for i in range(1,5):
            if list_scores[i-1] == 0:
                zero += 1
                if best_scorer == i:
                    zero += 1
            else:
